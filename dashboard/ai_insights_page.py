...
def render(df, sentiment_dist):
    render_section_header("🤖", "AI Recommendation Engine")
    st.caption("Data-driven recommendations to optimize your social media strategy.")

    recs = generate_recommendations(df, sentiment_dist)
    ...
    insights = [
        f"📌 You've analyzed **{total}** posts with an average engagement of **{avg_eng:,.0f}**.",
        f"🔥 **{viral}** posts ({viral/total*100:.1f}%) exceeded the viral threshold.",
    ]
    if top_cat:
