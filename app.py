...
with st.sidebar:
    ...
    data_source = st.radio(
        "Source", ["Default Dataset", "Database (Saved)", "Upload CSV", "Paste Raw CSV (AI Chat)"],
        label_visibility="collapsed",
        key="data_source"
    )
    ...
    if data_source == "Upload CSV":
        st.caption(f"Required: `Date`, `{cfg['post_label']}`, `{cfg['reactions_label']}`, `{cfg['shares_label']}`")
        
        template_cols = ["Date", cfg["post_label"], cfg["reactions_label"], cfg["shares_label"], cfg["comments_label"], cfg["impressions_label"], "Username", "Category", "Hashtags"]
        ...
        st.download_button(
            ...
            data=csv_template,
            file_name=f"socialpulse_template.csv",
            mime="text/csv",
            use_container_width=True
        )
        
        uploaded = st.file_uploader("Upload CSV", type=["csv"], label_visibility="collapsed")
    elif data_source == "Paste Raw CSV (AI Chat)":
        with st.expander("🤖 How to get data from AI Chat (Grok / ChatGPT)?"):
            st.markdown(f"""
            **1. Paste this into Grok, ChatGPT, Gemini, or Claude:**
            ```text
            {cfg['grok_prompt']}
            ```
            **2. Copy the CSV block generated by the AI and paste it below.**

            *Tip: Grok has real-time access to actual live posts, whereas ChatGPT, Gemini, and Claude are great at formatting text or generating highly realistic simulated datasets.*
            """)
...
def load_and_process(uploaded_file=None, raw_csv_text=None, platform="Twitter / X", use_db=False):
    """Full data pipeline: load → sentiment → engagement → fake detection."""
    from utils.db import load_from_db, save_dataframe_to_db

    if use_db:
        df = load_from_db(platform)
        if df.empty:
            raise ValueError("Database is empty for this platform. Please upload or paste a CSV first.")
    else:
        df = load_dataset(uploaded_file, raw_csv_text, platform=platform)
        # Save explicitly uploaded/pasted files to DB
        if uploaded_file is not None or (raw_csv_text is not None and raw_csv_text.strip() != ""):
            save_dataframe_to_db(df, platform)
...
# Load data
try:
    use_db = (data_source == "Database (Saved)")
    df = load_and_process(uploaded, raw_csv_text, platform=platform, use_db=use_db)
...
if not df.empty:
    with st.sidebar:
        st.markdown("<hr style='border-color:#2B3139;margin:20px 0;'>", unsafe_allow_html=True)
        st.markdown("<div style='color:#FCD535;font-weight:800;font-size:0.7rem;text-transform:uppercase;letter-spacing:1.5px;padding:10px 0;'>🔍 GLOBAL FILTERS</div>", unsafe_allow_html=True)
        ...
        max_eng = int(df["EngagementScore"].max()) if df["EngagementScore"].max() > 0 else 1000
        filter_min_eng = st.slider("Min Engagement Score", 0, max_eng, 0, step=max(1, max_eng//100))
    ...
    df = df[df["EngagementScore"] >= filter_min_eng]
...
if "Overview" in page:
    ...
    deepdive_page.render(df, platform)
